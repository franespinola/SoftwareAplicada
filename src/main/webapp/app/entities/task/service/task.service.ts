...
type RestOf<T extends ITask | NewTask> = Omit<T, 'createdAt' | 'targetDate'> & {
...
export class TaskService {
  ...
  protected convertDateFromClient<T extends ITask | NewTask | PartialUpdateTask>(task: T): RestOf<T> {
    return {
      ...
      targetDate: task.targetDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restTask: RestTask): ITask {
    return {
      ...
      targetDate: restTask.targetDate ? dayjs(restTask.targetDate) : undefined,
    };
  }
